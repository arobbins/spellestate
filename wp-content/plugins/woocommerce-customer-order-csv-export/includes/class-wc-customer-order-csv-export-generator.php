	...
	private function get_orders_csv_row( $order_id ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$line_item = array(
				...
				'refunded' => SV_WC_Plugin_Compatibility::is_wc_version_gte_2_2() ? wc_format_decimal( $order->get_total_refunded_for_item( $item_id ), 2 ) : '',
		...
		$order_data = array(
			...
			'refunded_total'       => SV_WC_Plugin_Compatibility::is_wc_version_gte_2_2() ? wc_format_decimal( $order->get_total_refunded(), 2 ) : '',
			'order_currency'       => $order->get_order_currency(),
			'payment_method'       => $order->payment_method,
			'shipping_method'      => $order->get_shipping_method(),
			'customer_id'          => SV_WC_Plugin_Compatibility::get_order_user_id( $order ),
