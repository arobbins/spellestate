...
"use strict";
			...
			curr.find('p span[class^=wcpfs_]').each( function() {
				...
				if ( $('.prdctfltr_customizer_fields').find('a.pf_active[data-filter="'+curr_label_stripped+'"]').length > 0 ) {
					...
					$('.prdctfltr_customizer div[data-filter="'+curr_label_stripped+'"] .pf_options_holder').append(curr_el.next().clone().wrap('<div>').parent().html());
						...
						curr_el.next().find('select, input').prop('disabled', true);
	...
	$(document).on('click', '.prdctfltr_c_add_filter', function() {
		...
		if ( curr_el.hasClass('pf_active') ) {
			...
			curr_set.next().find('select, input').prop('disabled', false);
			...
			curr_set.next().hide();
			curr_set.next().find('select, input').prop('disabled', true);
	...
	function makeVals(formControl, controlType, value) {
		switch (controlType) {
			...
			case 'select':
				...
				$(formControl).find('option[value="'+value+'"]').attr('selected',true);
				break;
			case 'multiselect':
		...
		$.post(prdctfltr.ajax, curr_data, function(response) {
			if (response) {
				...
				curr_form_el.each( function() {
					...
					curr_el.next().find('input, select').prop('disabled', false);
									...
									$.each(curr_preset['wc_settings_prdctfltr_active_filters'], function(index, pf_filter) {
										...
										if ( pf_filter == 'advanced' ) {
											...
											var curr_data = {
												...
												pfa_orderby: curr_preset['wc_settings_prdctfltr_advanced_filters']['pfa_orderby'] !== undefined ? curr_preset['wc_settings_prdctfltr_advanced_filters']['pfa_orderby'][curr] : '',
											...
											var curr_data = {
												...
												pfr_orderby: curr_preset['wc_settings_prdctfltr_range_filters']['pfr_orderby'] !== undefined ? curr_preset['wc_settings_prdctfltr_range_filters']['pfr_orderby'][zurr] : '',
												...
												pfr_custom: curr_preset['wc_settings_prdctfltr_range_filters']['pfr_custom'] !== undefined ? curr_preset['wc_settings_prdctfltr_range_filters']['pfr_custom'][zurr] : '',
												pfr_adoptive: curr_preset['wc_settings_prdctfltr_range_filters']['pfr_adoptive'] !== undefined ? curr_preset['wc_settings_prdctfltr_range_filters']['pfr_adoptive'][zurr] : 'no',
												pf_id: zurr
											};
