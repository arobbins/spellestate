...
/*			add_action( 'wp_ajax_nopriv_prdctfltr_respond', __CLASS__ . '::prdctfltr_respond' );
		...
		public static function prdctfltr_sc_products( $atts, $content = null ) {
			...
			echo self::get_pagination( $pagination_args );
			...
			return '<div' . ( $shortcode_id !== '' ? ' id="' . $shortcode_id.'"' : '' ) . ' class="prdctfltr_sc_products woocommerce' . ( $ajax == 'yes' ? ' prdctfltr_ajax' : '' ) . ( $fallback_css == 'yes' ? ' prdctfltr_fallback_css prdctfltr_columns_fallback_' . $columns : '' ) . ( $class !== '' ? ' ' . $class . '' : '' ) . '"' . $margin . ( $ajax == 'yes' ? $add_ajax : '' ) . '>' . do_shortcode( $cached ) . '</div>';
		...
		public static function make_query() {
			...
			if ( $opt['pf_set'] == 'shortcode' ) {
				if ( isset( $prdctfltr_global['pagefilters'][$prdctfltr_global['unique_id']]['atts']['columns'] ) ) {
				...
				$pf_col = ( $pf_col > 0 ? $pf_col : 0 );
				$pf_row = ( $pf_row > 0 ? $pf_row : 0 );
			...
			$prdctfltr_global['instance_data'] = array(
				'paged'			=> $opt['pf_paged'],
		...
		public static function get_pagination( $pagination ) {
			...
			if ( isset( $pagination['sc'] ) ) {
				...
				if ( $prdctfltr_global['pagination_type'] == 'default' ) {
					...
					if ( $ajax == 'yes' ) {
						add_filter( 'woocommerce_pagination_args', array( 'WC_Prdctfltr_Shortcodes', 'prdctfltr_pagination_filter' ), 999999, 1 );
					}

					global $wp_query;
					$wp_query->max_num_pages = self::$settings['instance']->max_num_pages;

					wc_get_template( 'loop/pagination.php' );

					/*remove_filter( 'woocommerce_pagination_args', array( 'WC_Prdctfltr_Shortcodes', 'prdctfltr_pagination_filter' ), 999999 );*/
		...
		public static function prdctfltr_sc_get_filter( $atts, $content = null ) {
			return include( WC_Prdctfltr::$dir . 'woocommerce/loop/product-filter.php' );
		}
		...
		function prdctfltr_pagination_filter( $args ) {
			...
			if ( isset( self::$settings['instance'] ) ) {
				$args['total'] = self::$settings['instance']->max_num_pages;
			}
