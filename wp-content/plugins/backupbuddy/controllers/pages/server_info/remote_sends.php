...
foreach( $remote_sends as $send_id => $remote_send ) {
  ...
  padding: 10px;">' . $remote_send['error'] . '</span>';
	...
	if ( 'success' != $remote_send['status'] ) {
		$status .= '<br><a title="' . __( 'Force resend attempt', 'it-l10n-backupbuddy' ) . '" href="' . pb_backupbuddy::ajax_url( 'remotesend_retry' ) . '&send_id=' . $send_id . '&#038;TB_iframe=1&#038;width=640&#038;height=600" class="thickbox">Retry Send Now</a>';
	}
