...
if ( $mode == 'add' ) { // ADD MODE.
	...
	if ( count( $settings_result ) == 0 ) { // No form submitted.
		...
		if ( count( $settings_result['errors'] ) > 0 ) { // Form errors.
			...
			$itxapi_token = ITXAPI_Helper2::get_wordpress_phpass( 'jwooley', $itxapi_password, 'http://jordan.dev.ithemes.com', '4.1' );//site_url(), $wp_version );
			$url_params = array(
				'action'    => 'connect',
				'user'      => $itxapi_username,
				'wp'        => $wp_version,
				'site'      => site_url(),
				'timestamp' => time()
			);
