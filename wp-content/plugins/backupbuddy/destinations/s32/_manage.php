...
if ( 'true' != pb_backupbuddy::_GET( 'listAll' ) ) {
	$listAll = '<small>[<a href="' . pb_backupbuddy::ajax_url( 'remoteClient' ) . '&destination_id=' . htmlentities( pb_backupbuddy::_GET( 'destination_id' ) ) . '&listAll=true">List all files in bucket (not just this site)</a>]</small>';
} else {
	$listAll = '<small>[<a href="' . pb_backupbuddy::ajax_url( 'remoteClient' ) . '&destination_id=' . htmlentities( pb_backupbuddy::_GET( 'destination_id' ) ) . '&listAll=false">Only list this site\'s files</a>]</small>';
}
pb_backupbuddy::$ui->title( __( 'S3 v2 Destination', 'it-l10n-backupbuddy' ) . ' "' . $destination['title'] . '" ' . $listAll );
?>
