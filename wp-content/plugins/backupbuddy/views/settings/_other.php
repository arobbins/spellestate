	...
	for ($x = 0; $x <= 3; $x++) { // Try this a few times since there may be race conditions on an open file.
		foreach( $files as $file ) {
			...
			if ( true !== ( $result = $backup_options->is_ok() ) ) {
				...
				if ( ( FALSE !== $backup_options->options['finish_time'] ) && ( '-1' != $backup_options->options['finish_time'] ) ) {
...
<div>
	<a href="<?php echo pb_backupbuddy::page_url(); ?>&cleanup_now=true&tab=3" class="button secondary-button"><?php _e('Cleanup OLD temporary data', 'it-l10n-backupbuddy' );?>*</a>
	&nbsp;
