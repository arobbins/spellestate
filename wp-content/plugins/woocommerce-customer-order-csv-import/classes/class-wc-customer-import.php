...
class WC_CSV_Customer_Import extends WP_Importer {
	...
	private function import_options() {
		...
		<form action="<?php echo admin_url( 'admin.php?import=woocommerce_customer_csv&step=2' ); ?>" method="post">
			...
			<h3><?php _e( 'Shipping Address', 'woocommerce-customer-order-csv-import' ); ?></h3>
			<p>
				<input type="checkbox" value="1" name="billing_address_for_shipping_address" id="billing_address_for_shipping_address" />
				<label for="billing_address_for_shipping_address"><?php _e( 'Use billing address as shipping address if not set', 'woocommerce-customer-order-csv-import' ); ?></label>
			</p>
			...
			<p>
				<label for="hashed_passwords"><?php _e( "Don't hash user passwords", 'woocommerce-customer-order-csv-import' ) ?></label>
				<input type="checkbox" value="1" name="hashed_passwords" id="hashed_passwords" />
				<span class="description"><?php _e( 'Enable this if your customer import file contains passwords which are already correctly hashed for WordPress.', 'woocommerce-customer-order-csv-import' ); ?></span>
			</p>
			<p>
	...
	private function import_end() {
		...
		echo '<p>' . __( 'All done!', 'woocommerce-customer-order-csv-import' ) . ' <a href="' . network_admin_url( 'users.php' ) . '">' . __( 'View Users', 'woocommerce-customer-order-csv-import' ) . '</a>' .
	...
	private function process_users() {
		...
		$this->log->add( '---' );
		$this->log->add( __( 'Processing customers.', 'woocommerce-customer-order-csv-import' ) );
		...
		foreach ( $this->users as $user ) {
			...
			if ( ! empty( $user['id'] ) && isset( $this->processed_users[ $user['id'] ] ) ){
				$this->skipped++;
				$this->log->add( __( '> Customer already processed. Skipping.', 'woocommerce-customer-order-csv-import' ), true );
				continue;
			}
			...
			if ( ! $merging ) {
				...
				if ( $user_exists ) {
					$this->skipped++;
					$this->log->add( sprintf( __( '> %s already exists.', 'woocommerce-customer-order-csv-import' ), esc_html( isset( $user['username'] ) && $user['username'] ? $user['username'] : $user['email'] ) ), true );
					continue;
				}
			}
			...
			if ( ! validate_username( $user['username'] ) ) {
				$this->skipped++;
				$this->log->add( sprintf( __( '> Invalid username: %s.', 'woocommerce-customer-order-csv-import' ), esc_html( $user['username'] ) ), true );
				continue;
			}
			...
			if ( ! is_email( $user['email'] ) ) {
				$this->skipped++;
				$this->log->add( sprintf( __( '> Invalid email: %s.', 'woocommerce-customer-order-csv-import' ), esc_html( $user['email'] ) ), true );
				continue;
			}
			...
			if ( $merging ) {
				...
				$this->log->add( sprintf( __( '> Merging customer ID %s.', 'woocommerce-customer-order-csv-import' ), $user_id ), true );
				...
				if ( ! $dry_run ) {
					...
					if ( is_wp_error( $user_id ) ) {
						...
						/* translators: Placeholders: %1$s - username, %2$s - error message */
						$this->log->add( sprintf( __( 'Failed to update customer %1$s: %2$s;', 'woocommerce-customer-order-csv-import' ), esc_html( $user['username'] ), esc_html( $user_id->get_error_message() ) ) );
						continue;
				...
				if ( isset( $user['email'] ) && $user['email'] && email_exists( $user['email'] ) ) {
					$this->log->add( sprintf( __( '> > Skipped. Email %s already in use.', 'woocommerce-customer-order-csv-import' ), $user['email'] ) );
					$this->skipped++;
					continue;
				}
				...
				if ( isset( $user['username'] ) && $user['username'] && username_exists( $user['username'] ) ) {
					$this->log->add( sprintf( __( '> > Skipped. Username %s already in use.', 'woocommerce-customer-order-csv-import' ), $user['username'] ) );
					$this->skipped++;
					continue;
				}

				// Insert customer
				$this->log->add( sprintf( __( '> Inserting %s', 'woocommerce-customer-order-csv-import' ), esc_html( $user['username'] ) ), true );
				...
				if ( ! $dry_run ) {
					...
					if ( is_wp_error( $user_id ) ) {
						...
						/* translators: Placeholders: %1$s - username, %2$s - error message */
						$this->log->add( sprintf( __( 'Failed to import customer %1$s: %2$s;', 'woocommerce-customer-order-csv-import' ), esc_html( $user['username'] ), esc_html( $user_id->get_error_message() ) ) );
						continue;

					} else {
						// success
						$this->log->add( sprintf( __( '> Inserted - customer ID is %s.', 'woocommerce-customer-order-csv-import' ), $user_id ) );
			...
			if ( $merging ) {
				$this->merged++;
				$this->log->add( sprintf( __( '> Finished merging customer ID %s.', 'woocommerce-customer-order-csv-import' ), $user_id ) );
			} else {
				$this->imported++;
				$this->log->add( sprintf( __( '> Finished importing customer ID %s.', 'woocommerce-customer-order-csv-import' ), $user_id ) );
			}
		}

		$this->log->add( __( 'Finished processing customers.', 'woocommerce-customer-order-csv-import' ) );
	...
	private function header() {
		...
		echo '<h2>' . __( 'Import Customers', 'woocommerce-customer-order-csv-import' ) . '</h2>';
	}
	...
	private function greet() {
		...
		echo '<p>' . __( 'Hi there! Upload a CSV file containing customer data to import the contents into your shop.', 'woocommerce-customer-order-csv-import' ) . '</p>';
