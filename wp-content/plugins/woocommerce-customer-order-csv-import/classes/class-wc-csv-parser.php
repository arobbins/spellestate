...
class WC_CSV_Customer_Parser {
	...
	function parse( $file, $delimiter ) {
		...
		if ( $dry_run ) {
			...
			/* translators: Placeholders: %s - the import type: one of customer, order, or coupon */
			$WC_CSV_Import->log->add( sprintf( __( 'Dry Run: no %s will be imported', 'woocommerce-customer-order-csv-import' ), $this->type ) );
		}

		/* translators: Placeholders: %s - the import type: one of customer, order, or coupon */
		$WC_CSV_Import->log->add( sprintf( __( 'Parsing %s CSV.', 'woocommerce-customer-order-csv-import' ), $this->type ) );
		...
		/* translators: Placeholders: %s - the import type: one of customer, order, or coupon */
		$WC_CSV_Import->log->add( sprintf( __( 'Finished parsing %s CSV.', 'woocommerce-customer-order-csv-import' ), $this->type ) );
	...
	private function parse_coupons( $parsed_data, $raw_headers, $merging, $record_offset ) {
		...
		foreach ( $raw_headers as $key => $value ) {
			if ( 'meta:' != substr( $key, 0, 5 ) ) {
				if ( ! in_array( $key, $this->coupon_data_fields ) && ! in_array( $key, $this->coupon_meta_fields ) ) {
					$WC_CSV_Import->log->add( sprintf( __( "> Warning: Unknown column named '%s' will be ignored.", 'woocommerce-customer-order-csv-import' ), esc_html( $value ) ) );
				}
			}
		}
		...
		foreach ( $parsed_data as $item ) {
			...
			if ( $row <= $record_offset ) {
				$WC_CSV_Import->log->add( sprintf( __( '> Row %s - skipped due to record offset.', 'woocommerce-customer-order-csv-import' ), $row ) );
				continue;
			}

			$postmeta = $coupon = array();

			// log the line number and coupon code
			/* translators: Placeholders: %1$s - row number, %2$s - coupon code if available */
			$WC_CSV_Import->log->add( sprintf( __( '> Row %1$s%2$s - preparing for import.', 'woocommerce-customer-order-csv-import' ), $row, ( isset( $item['coupon_code'] ) && $item['coupon_code'] ? ' - "' . $item['coupon_code'] . '"' : '' ) ) );

			// coupon code is required for merging or updating
			if ( ! isset( $item['coupon_code'] ) || ! $item['coupon_code'] ) {
				$WC_CSV_Import->log->add( __( '> > Skipped. Missing coupon code.', 'woocommerce-customer-order-csv-import' ) );
				$skipped++;
				continue;
			}
			...
			if ( $merging ) {
				...
				if ( ! $coupon_found ) {
					$WC_CSV_Import->log->add( sprintf( __( "> > Skipped. Coupon code '%s' not found, unable to merge.", 'woocommerce-customer-order-csv-import' ), esc_html( $item['coupon_code'] ) ) );
					$skipped++;
					continue;
				}
				...
				if ( $coupon_found ) {
					$WC_CSV_Import->log->add( sprintf( __( "> > Skipped. Coupon code '%s' already exists.", 'woocommerce-customer-order-csv-import' ), esc_html( $item['coupon_code'] ) ) );
					$skipped++;
					continue;
				}
			}
			...
			if ( ! $merging && ( ! isset( $item['discount_type'] ) || ! $item['discount_type'] ) ) {
				$WC_CSV_Import->log->add( __( '> > Skipped. Missing discount type.', 'woocommerce-customer-order-csv-import' ) );
				$skipped++;
				continue;
			}
			...
			if ( isset( $item['discount_type'] ) && $item['discount_type'] ) {
				...
				foreach ( $discount_types as $key => $value ) {
					if ( 0 === strcasecmp( $key, $item['discount_type'] ) || 0 === strcasecmp( $value, __( $item['discount_type'], 'woocommerce-customer-order-csv-import' ) ) ) {
				...
				if ( ! $discount_type_is_valid ) {
					$WC_CSV_Import->log->add( sprintf( __( "> > Skipped. Unknown discount type '%s'.", 'woocommerce-customer-order-csv-import' ), esc_html( $item['discount_type'] ) ) );
					$skipped++;
					continue;
				}
			}
				...
				switch ( $column ) {
					...
					case 'exclude_products':
						...
						foreach ( $skus as $sku ) {
							...
							if ( ! $product_id ) {
								// unknown product
								$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Unknown product sku: %s.', 'woocommerce-customer-order-csv-import' ), esc_html( $sku ) ) );
								$skipped++;
					...
					case 'exclude_product_categories':
						...
						foreach ( $product_cats as $product_cat ) {
							...
							if ( ! $term ) {
								// unknown category
								$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Unknown product category: %s.', 'woocommerce-customer-order-csv-import' ), esc_html( $product_cat ) ) );
								$skipped++;
					...
					case 'customer_emails':
						...
						foreach ( $emails as $email ) {
							if ( ! is_email( $email ) ) {
								// invalid email
								$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Invalid email: %s.', 'woocommerce-customer-order-csv-import' ), esc_html( $email ) ) );
								$skipped++;
								continue 4;  // continue main loop
							}
							$val[] = $email;
					...
					case 'exclude_sale_items':
						...
						if ( $val && 'yes' != $val && 'no' != $val ) {
							$WC_CSV_Import->log->add( sprintf( __( "> > Skipped. Column '%s' must be 'yes' or 'no'.", 'woocommerce-customer-order-csv-import' ), esc_html( $raw_headers[ $column ] ) ) );
							$skipped++;
					...
					case 'expiry_date':
						...
						if ( $val && false === ( $val = strtotime( $val ) ) ) {
							// invalid date format
							$WC_CSV_Import->log->add( sprintf( __( "> > Skipped. Invalid date format '%s'.", 'woocommerce-customer-order-csv-import' ),  esc_html( $item[ $column ] ) ) );
							$skipped++;
							continue 3;  // continue main loop
						}
					...
					case 'limit_usage_to_x_items':
						...
						if ( '' !== $val && ! is_numeric( $val ) ) {
							...
							$WC_CSV_Import->log->add( sprintf( __( "> > Skipped. Invalid %1$s '%2$s'.", 'woocommerce-customer-order-csv-import' ), esc_html( $raw_headers[ $column ] ), esc_html( $val ) ) );
							$skipped++;
							continue 3;  // continue main loop
						}
						$postmeta[] = array( 'key' => $column, 'value' => $val );
	...
	private function wc_get_order_statuses_clean() {
	...
	private function parse_orders( $parsed_data, $raw_headers, $merging, $record_offset ) {
		...
		$shop_order_status = $this->wc_get_order_statuses_clean();
		...
		foreach ( $parsed_data as $item ) {
			...
			if ( ! $csv_export_file ) {
				...
				$WC_CSV_Import->log->add( sprintf( __( '> Row %s - preparing for import.', 'woocommerce-customer-order-csv-import' ), $row ) );
				...
				if ( $order_number && ! is_numeric( $order_number ) ) {
					...
					$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Order number field must be an integer: %s.', 'woocommerce-customer-order-csv-import' ),  $order_number ) );
					$skipped++;
					continue;
				...
				if ( $order_number_formatted && ! $order_number ) {
					...
					$WC_CSV_Import->log->add( __( '> > Skipped. Formatted order number provided but no numerical order number, see the documentation for further details.', 'woocommerce-customer-order-csv-import' ) );
					$skipped++;
					continue;
			...
			if ( $order_number_formatted ) {
				...
				if ( $order_id ) {
					$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Order %s already exists.', 'woocommerce-customer-order-csv-import' ), $order_number_formatted ) );
					$skipped++;
					continue;
				}
			}
			...
			if ( isset( $item['customer_user'] ) && $item['customer_user'] ) {
				...
				if ( is_int( $item['customer_user'] ) ) {
					...
					if ( ! $found_customer ) {
						...
						$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Cannot find customer with id %s.', 'woocommerce-customer-order-csv-import' ),  $item['customer_user'] ) );
						$skipped++;
						continue;
			...
			if ( ! empty( $item['status'] ) ) {
				...
				if ( ! $found_status ) {
					...
					$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Unknown order status %1$s (%2$s).', 'woocommerce-customer-order-csv-import' ),  $item['status'], implode( $available_statuses, ', ' ) ) );
					$skipped++;
					continue;
				}
			} else {
			...
			if ( ! empty( $item['date'] ) ) {
				if ( false === ( $item['date'] = strtotime( $item['date'] ) ) ) {
					// invalid date format
					$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Invalid date format %s.', 'woocommerce-customer-order-csv-import' ),  $item['date'] ) );
					$skipped++;
					continue;
				}
			} else {
			...
			$order['order_comments'] = ! empty( $item['customer_note'] ) ? $item['customer_note'] : '';
			...
			if ( ! $csv_export_file ) {
				...
				if ( ! empty( $item['order_item_1'] ) ) {
					...
					while ( ! empty( $item[ 'order_item_' . $i ] ) ) {
						...
						if ( ! $product_identifier || ! $qty || ! is_numeric( $total ) ) {
							// invalid item
							$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Missing SKU, quantity or total for %s on row %s.', 'woocommerce-customer-order-csv-import' ),  'order_item_' . $i, $row ) );
							$skipped++;
						...
						if ( ! $allow_unknown_products && ! $product_id ) {
							// unknown product
							$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Unknown order item: %s.', 'woocommerce-customer-order-csv-import' ),  $product_identifier ) );
							$skipped++;
							continue 2;  // break outer loop
						}
				...
				if ( ! $sku || ! $qty || ! is_numeric( $total ) ) {
					// invalid item
					$WC_CSV_Import->log->add( sprintf( __( '> > Row %d - %s - skipped. Missing SKU, quantity or total', 'woocommerce-customer-order-csv-import' ), $row, $item['order_id'] ) );
					$skipped++;
				...
				if ( ! $product_id ) {
					...
					/* translators: Placeholders: %1$d - row number, %2$s - order id, %3$s - order item SKU */
					$WC_CSV_Import->log->add( sprintf( __( '> > Row %1$d - %2$s - skipped. Unknown order item: %3$s.', 'woocommerce-customer-order-csv-import' ), $row, $item['order_id'], $sku ) );
					$skipped++;
					continue;  // break outer loop
				}
	...
	private function parse_customers( $parsed_data, $raw_headers, $merging, $record_offset ) {
		...
		foreach ( $parsed_data as $item ) {
			...
			if ( $merging ) {
				...
				$WC_CSV_Import->log->add( sprintf( __( '> Row %s - preparing for merge.', 'woocommerce-customer-order-csv-import' ), $row ) );

				// Required fields
				if ( ! $customer_id && empty( $item['email'] ) && empty( $item['username'] ) ) {
					$WC_CSV_Import->log->add( __( '> > Skipped. Cannot merge without id, email or username.', 'woocommerce-customer-order-csv-import' ) );
					$skipped++;
					continue;
				}
				...
				if ( $customer_id ) {
					...
					if ( ! $found_customer ) {
						...
						$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Cannot find customer with id %s.', 'woocommerce-customer-order-csv-import' ),  $customer_id ) );
						$skipped++;
						continue;

					}

					$WC_CSV_Import->log->add( sprintf( __( '> > Found user with ID %s.', 'woocommerce-customer-order-csv-import' ), $customer_id ) );
					...
					if ( ! $found_customer ) {
						...
						$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Cannot find customer with username %s.', 'woocommerce-customer-order-csv-import' ), $item['username'] ) );
						$skipped++;
						continue;
					}

					$WC_CSV_Import->log->add( sprintf( __( '> > Found user with username %s.', 'woocommerce-customer-order-csv-import' ), $item['username'] ) );
					...
					if ( ! $found_customer ) {
						...
						$WC_CSV_Import->log->add( sprintf( __( '> > Skipped. Cannot find customer with email %s.', 'woocommerce-customer-order-csv-import' ), $item['email'] ) );
						$skipped++;
						continue;
					}

					$WC_CSV_Import->log->add( sprintf( __( '> > Found user with email %s.', 'woocommerce-customer-order-csv-import' ), $item['email'] ) );
					$customer_id = $found_customer;
				}
				...
				if ( empty( $item['email'] ) ) {
					$WC_CSV_Import->log->add( __( '> > Skipped. No email set for new customer.', 'woocommerce-customer-order-csv-import' ) );
					$skipped++;
					continue;
				}
			}
			...
			if ( isset( $item['role'] ) && $item['role'] ) {
				...
				if ( ! isset( $wp_roles->role_names[ $item['role'] ] ) ) {
					...
					if ( ! $found_role_by_name ) {
						$WC_CSV_Import->log->add( sprintf( __( "> > Skipped. Role '%s' not found.", 'woocommerce-customer-order-csv-import' ), $item['role'] ) );
						$skipped++;
						continue;
					}
				}
