...
class WC_CSV_Import_Suite_Importer extends WP_Importer {
	...
	/** @var bool whether file url imports are enabled or not */
	// TODO: implement this
	 ...
	 *
	 * @since 3.0.0
	 * @param array $data Job data for WC_CSV_Import_Suite_Background_Import
	 * @param array $options Job options
	 */
	private function start_background_import( $data, $options ) {

		$background_jobs = wc_csv_import_suite()->get_background_import_instance();

		$job = $background_jobs->create_job( $data, $options );
		$background_jobs->dispatch();

		$redirect_to = admin_url( 'admin.php?import=' . $data['type'] . '&job_id=' . urlencode( $job->id ) );
	...
	protected function render_import_progress( $job_id ) {
		...
		$filename = substr( $job->data['file'], strrpos( $job->data['file'], '/' ) + 1 );

		$legends = array(

			'inserted' => array(
				'title' => sprintf( $this->i18n['count_inserted'], '<strong><span class="amount">0</span></strong> ' ),
			...
			'merged' => array(
				'title' => sprintf( $this->i18n['count_merged'], '<strong><span class="amount">0</span></strong> ' ),
			...
			'skipped' => array(
				'title' => sprintf( $this->i18n['count_skipped'], '<strong><span class="amount">0</span></strong> ' ),
			...
			'failed' => array(
				'title' => sprintf( $this->i18n['count_failed'], '<strong><span class="amount">0</span></strong> ' ),
		...
		wc_enqueue_js( "
			wc_csv_import_suite.bytes = " . (int) $job->data['bytes'] . ";
			wc_csv_import_suite.dry_run = " . ( ( (bool) $job->options['dry_run'] ) ? 'true' : 'false' ) . ";
			wc_csv_import_suite.job_id = '" . esc_html( $job_id ) . "';
			wc_csv_import_suite.run_live_url = '" . esc_html( $run_live_url ) . "';
			wc_csv_import_suite.legends = " . json_encode( $legends ) .  ";
			wc_csv_import_suite.i18n.chart_tooltip = '" . $this->i18n['count'] . "';
			wc_csv_import_suite.display_import_progress();
		" );
	}
					...
					if ( isset( $this->inserted_terms[ $taxonomy ][ $parent ][ $term ] ) ) {
						...
						if ( ! $term_id ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								wc_csv_import_suite()->log( sprintf( __( '> > (' . $this->line_num . ') Failed to import term %s, parent %s - %s', 'woocommerce-csv-import-suite' ), sanitize_text_field( $term ), sanitize_text_field( $parent ), sanitize_text_field( $taxonomy ) ) );
								break;
							}
						}
	 ...
	 */
	protected function add_import_result( $status, $message = '', $log = true ) {

		$this->import_results[ $this->line_num ] = array(
			'status'  => $status,
			'message' => $message,
		);
	...
	/**
	 * Get all error messages from WP_Error and glue them together
	 *
	 * @since 3.0.0
