...
class WC_CSV_Import_Suite_Customer_Import extends WC_CSV_Import_Suite_Importer {
		...
		<tr>
			...
			<td>
				<label>
					...
					<?php _e( 'Send welcome emails to new customers after import.', 'woocommerce-csv-import-suite' ); ?>
				</label>
			</td>
		</tr>
	...
	protected function process_item( $data, $options = array(), $raw_headers = array() ) {
		...
		if ( $merging ) {
			...
			wc_csv_import_suite()->log( sprintf( __( '> Merging customer ID %s.', 'woocommerce-csv-import-suite' ), $id ) );
	...
	public function update_customer( $id, $data, $options ) {
		...
		try {
			...
			if ( isset( $data['password'] ) ) {
				$user_data['user_pass'] = wc_clean( $data['password'] );
			}

			if ( ! empty( $user_data ) ) {

				$user_data['ID'] = intval( $id );
	...
	/**
	 * Force generata a password for a new customer
	 *
	 * @since 3.0.0
