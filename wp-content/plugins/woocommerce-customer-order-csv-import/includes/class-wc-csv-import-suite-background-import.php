...
class WC_CSV_Import_Suite_Background_Import extends SV_WP_Background_Job_Handler {
	...
	protected function process_job( $job ) {
		...
		$importer = wc_csv_import_suite()->get_importers_instance()->get_importer( $job->data['type'] );

		// no importer found, not much we can do here, halt further processing
		if ( ! $importer ) {

			$message = sprintf( esc_html__( 'Unknown importer "%s". Cancelling.', 'woocommerce-csv-import-suite' ), $job->data['type'] );
		...
		while ( is_numeric( $start_pos ) && $start_pos <= $job->data['bytes'] ) {
			...
			$importer->import( $job->data['file'], $options );
		...
		// job complete! :)
		if ( ! is_numeric( $start_pos ) || $start_pos >= $job->data['bytes'] ) {
	 ...
	 * @since 3.0.0
	 */
	protected function task( $item, $options = null ) {
		// no-op
	}
