<?php
    ...
    // Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework_extension_custom_icon_fonts' ) ) {
        /**
         * Main ReduxFramework custom_icon_fonts extension class
         *
         * @since       3.1.6
         */
        class ReduxFramework_extension_custom_icon_fonts {
            ...
            /**
             * @var ReduxFramework_extension_custom_icon_fonts
             */
             ...
             * @var array
             */
            public $custom_icon_fonts = array();
            ...
            public function __construct( $parent ) {
                ...
                $this->upload_dir = ReduxFramework::$_upload_dir . '/custom-icon-fonts/';
                $this->upload_url = ReduxFramework::$_upload_url . '/custom-icon-fonts/';
                ...
                $this->field_name = 'custom_icon_fonts';
                ...
                add_action( 'wp_ajax_redux_custom_icon_fonts', array( $this, 'ajax' ) );
                add_action( 'wp_ajax_redux_custom_font_timer', array( $this, 'timer' ) );

                add_filter( "redux/{$this->parent->args['opt_name']}/field/typography/custom_icon_fonts", array( $this, 'addCustomFonts' ) );

                $this->is_field = Redux_Helpers::isFieldInUse( $parent, 'custom_icon_fonts' );
            ...
            function extend_tinymce_dropdown( $opt ) {
                ...
                if (file_exists($this->upload_dir . 'fonts.css')) {
                    ...
                    foreach ( $this->custom_icon_fonts as $title => $arr ) {
                        foreach($arr as $font => $pieces) {
                            $custom_icon_fonts .= ';' . $font . '=' . $font;
                        }

                        continue;
                    }

                    $opt['font_formats'] = $theme_advanced_fonts . $custom_icon_fonts;
                }
            ...
            /**
             * Gets all the fonts in the custom_icon_fonts directory
             */
            public function getFonts() {
                if ( ! empty( $this->custom_icon_fonts ) ) {
                    return $this->custom_icon_fonts;
                }
                    ...
                    foreach ( $fonts as $section ) {
                        if ( $section['type'] == "d" && ! empty( $section['name'] ) ) {
                            if ( $section['name'] == "custom" ) {
                                $section['name'] = __( 'Custom Icon Fonts', 'redux-framework' );
                            ...
                            $this->custom_icon_fonts[ $section['name'] ] = isset( $this->custom_icon_fonts[ $section['name'] ] ) ? $this->custom_icon_fonts[ $section['name'] ] : array();
                            ...
                            foreach ( $section['files'] as $font ) {
                                if ( ! empty( $font['name'] ) ) {
                                    ...
                                    $this->custom_icon_fonts[ $section['name'] ][ $font['name'] ] = $kinds;
                                }
                            }
                        }
                    }
            ...
            /**
             * @param $custom_icon_fonts
             *
             * @return array
             */
            public function addCustomFonts( $custom_icon_fonts ) {
                if ( ! is_array( $custom_icon_fonts ) || empty( $custom_icon_fonts ) ) {
                    $custom_icon_fonts = array();
                }
                
                $custom_icon_fonts = wp_parse_args( $custom_icon_fonts, $this->custom_icon_fonts );

                return $custom_icon_fonts;
            }
            ...
            public function ajax() {
                if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], "redux_{$this->parent->args['opt_name']}_custom_icon_fonts" ) ) {
                    die( 0 );
                }

                if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "delete" ) {
                    if ( $_REQUEST['section'] == __( 'Custom Icon Fonts', 'redux-framework' ) ) {
                    ...
                    try {
                        ...
                        $name_pos = strrpos($_REQUEST['name'], "fontello");
 
                        if ( $name_pos >= 0 ) {     
                            delete_option('sf_fontello_icon_codes');
                            $this->parent->filesystem->execute('rmdir', $this->upload_dir . '/fontello_css/', array('recursive' => true));         
                        }
            ...
            function processWebfont( $attachment_id, $name, $mime_type, $subfolder = 'custom/' ) {
                ...
                if ( $subtype == "zip" ) {
                    ...
                    $name_pos = strrpos($name, "fontello");

                    if ( $name_pos >= 0 ) {     
                    
                        //Fontello Fonts Processment
                        $fontello_path_file = $temp . '/' .  $name . '/config.json';

                        $fontello_css_path_file = $temp . '/' .  $name . '/css/fontello-embedded.css';
                        $fontello_codes = array();

                        $fontello_data = json_decode( file_get_contents(  $fontello_path_file ) );
                        
                        foreach ($fontello_data->glyphs as $icon_key){
                                array_push($fontello_codes , $icon_key->css);
                        }

                        update_option('sf_fontello_icon_codes', $fontello_codes) ;
                        
                         if ( ! is_dir( $this->upload_dir . '/fontello_css/'  ) ) {
                                $this->parent->filesystem->execute( 'mkdir', $this->upload_dir . '/fontello_css/' );
                                
                                if (!copy($fontello_css_path_file, $this->upload_dir . '/fontello_css/fontello-embedded.css' ) ) {
                                   echo json_encode( array(
                                        'type' => 'error',
                                        'msg'  => __( 'Error when creating Fontello file.', 'redux-framework' )
                                        ) ); 
                                    }
                            }
                    }
            ...
            /**
             * @return ReduxFramework_extension_custom_icon_fonts
             */
