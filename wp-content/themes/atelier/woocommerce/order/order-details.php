...
<div class="my-account-right">
	...
	<table class="shop_table order_details">
								...
								<?php
									...
									if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
										...
										foreach ( $download_files as $download_id => $file ) {
											...
											$links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( __( 'Download file%s', 'swiftframework' ), ( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
										}
		...
		<?php
					...
					if ( $has_refund && $key === 'order_total' ) {
						...
						if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
							...
							if ( ! empty( $tax_del_array ) ) {
								$refunded_tax_del .= ' ' . sprintf( __( '(Includes %s)', 'swiftframework' ), implode( ', ', $tax_del_array ) );
							}

							if ( ! empty( $tax_ins_array ) ) {
								$refunded_tax_ins .= ' ' . sprintf( __( '(Includes %s)', 'swiftframework' ), implode( ', ', $tax_ins_array ) );
							}
						}
			...
			if ( $has_refund ) { ?>
				<tr>
					<th scope="row"><?php _e( 'Refunded:', 'swiftframework' ); ?></th>
			...
			if ( '' != $order->customer_note ) { ?>
				<tr>
					<th scope="row"><?php _e( 'Note:', 'swiftframework' ); ?></th>
	...
	<header>
		<h2><?php _e( 'Customer details', 'swiftframework' ); ?></h2>
	</header>
	<table class="shop_table shop_table_responsive customer_details">
	<?php
		if ( $order->billing_email ) {
			echo '<tr><th>' . __( 'Email:', 'swiftframework' ) . '</th><td data-title="' . __( 'Email', 'swiftframework' ) . '">' . $order->billing_email . '</td></tr>';
		}

		if ( $order->billing_phone ) {
			echo '<tr><th>' . __( 'Telephone:', 'swiftframework' ) . '</th><td data-title="' . __( 'Telephone', 'swiftframework' ) . '">' . $order->billing_phone . '</td></tr>';
		}
	...
	<?php endif; ?>
			...
			<header class="title">
				<h3><?php _e( 'Billing Address', 'swiftframework' ); ?></h3>
			</header>
			<address>
				<?php
	...
	<?php if ( ! wc_ship_to_billing_address_only() && $order->needs_shipping_address() && get_option( 'woocommerce_calc_shipping' ) !== 'no' ) : ?>
		...
		<div class="col-2">
			...
			<header class="title">
				<h3><?php _e( 'Shipping Address', 'swiftframework' ); ?></h3>
			</header>
			<address>
				<?php
